...
func readpk(var path):
	...
	info["ev_hp"] = read_8(file,0x26)
	info["ev_atk"] = read_8(file,0x27)
	info["ev_def"] = read_8(file,0x28)
	info["ev_spe"] = read_8(file,0x29)
	info["ev_spa"] = read_8(file,0x2A)
	info["ev_spd"] = read_8(file,0x2B)
	var ability_num = read_8(file,0x16)
	bin = BinaryTranslator.int_to_bin(ability_num)
	bin = BinaryTranslator.bitshiftR(bin,1)
	info["ability_number"] = BinaryTranslator.bin_to_int(bin)
	...
	bin = BinaryTranslator.int_to_bin(read_8(file,0x125))
...
func writepk(path,info):
	...
	write_16(file,0x72,info["move1"]["id"])
	write_16(file,0x74,info["move2"]["id"])
	write_16(file,0x76,info["move3"]["id"])
	write_16(file,0x78,info["move4"]["id"])
