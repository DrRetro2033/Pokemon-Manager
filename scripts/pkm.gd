...
func _ready():
	print(Pokemon.experience(46,"medium-slow"))
...
func get_info(user_pokemon, existing_pokemon):
	...
	while user_pokemon.size() > 0:
		...
		if array == null: #If array is null, then that means that the file being read is not a vaild pkm file.
			$"Loading Screen/CanvasLayer/ProgressBar".max_value -= 1 
			#Makes sure that the progress bar is still moving and will fill completly.
			continue
		if existing_pokemon != null and not existing_pokemon.data.empty() and pokemon.has(array["id"]): 
			#if the pokemon is already in the database, skip it
				id += 1
				continue
		elif array != null and not array.empty(): 
			#asks PokeAPI for infomation about the pokemons species and known moves.
...
func _on_Calculator_pressed():
	var window = load("res://scenes/Calculator.tscn").instance()
